/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IntegrationTestWhereInput } from "./IntegrationTestWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { IntegrationTestOrderByInput } from "./IntegrationTestOrderByInput";

@ArgsType()
class IntegrationTestFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => IntegrationTestWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => IntegrationTestWhereInput, { nullable: true })
  @Type(() => IntegrationTestWhereInput)
  where?: IntegrationTestWhereInput;

  @ApiProperty({
    required: false,
    type: [IntegrationTestOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [IntegrationTestOrderByInput], { nullable: true })
  @Type(() => IntegrationTestOrderByInput)
  orderBy?: Array<IntegrationTestOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { IntegrationTestFindManyArgs as IntegrationTestFindManyArgs };
